...
\begin{problem}
Suppose the temperature at a point in the plane is given by the function {$T(x,y)=x^2-xy-y^2$} degrees Fahrenheit. A particle is at $(2,3)$.  
\begin{enumerate}
 \item Use differentials to estimate the change in temperature if the particle moves 1 unit in the direction of $\left(3,4\right)$.
 \item What is the actual change in temperature if the particle moves 1 unit in the direction of $\left(3,4\right)$?
 \item Use differentials to estimate the change in temperature if the particle moves about .2 unit in the direction of $\left(3,4\right)$.
\end{enumerate}
\end{problem}
...
\begin{enumerate}
 ...
 \item Consider the direction $\vec A=(3,4)$.  Find a unit vector in the direction of $\vec u$.  If we move one unit in the direction of $\vec A$, what is $dz$? [It's O.K. to leave you answer as a dot product.]    
\end{enumerate}
...
\begin{problem}
 ...
 \begin{enumerate}
  ...
  \item At the point $(2,1)$, place a dot on your graph. Then draw a unit vector based at $(2,1)$ that points in the direction $(3,4)$. If you were to move in the direction of $(3,4)$, starting from the point $(2,1)$, would the value of $f$ increase or decrease?
  \item Find the slope of $f$ at $(2,1)$ in the direction of $(3,4)$ by finding the directional derivative. This should agree with your previous answer.
  \item If you stand at $(-2,3)$ and move in the direction $(1,-1)$, will $f$ increase or decrease?  Find the directional derivative of $f$ in the direction $(1,-1)$ at the point $(-2,3)$.
 \end{enumerate}
\end{problem}
...
% \item If a contour line intersects itself, what does that mean? I'll do this one in class.
 \item \marginparbmw{If you're stuck, look at a contour plot of $f(x,y) = (x+1)^3-3(x+1)^2-y^2+2$ in   
\href{http://www.wolframalpha.com/input/?i=contourplot+\%28x+\%2B+1\%29^3+-+3*\%28x+\%2B+1\%29^2+-+y^2+\%2B+2+for+x+from+-2+to+2+and+y+from+-2+to+2}{Wolfram Alpha}. Then make your own example.}%
...
\marginpar{In this example, the second derivative is constant, so the point $(4/3,-2/3)$ did not change the matrix. In general, the point will affect your matrix. The following graph of $f$ shows the eigenvectors (in red) in which the largest and smallest second derivatives occur.   
