...
\begin{problem}\marginpar{Use \href{http://sagecell.sagemath.org/?z=eJwr0ijVKdO0BZE6pVrJ-cUaZZqavFy8XAVFmXklCkohGakKKalFmWWJJZllqQqZxUogySK9lMy0NA1NsGYAokcTpg==&lang=sage}{Sage} to check your work in each part.}%
Compute the partial derivatives and then state the total derivatives for the functions below.  Also, for each function, tell what the range and domain are (i.e., $\mathbb{R}^?\to\mathbb{R}^?$).
\begin{enumerate}
 \item  The parametric surface $\vec r(u,v) = (u,v,u\cos(v))$. Compute $\vec r_u$, $\vec r_v$, and $D\vec r(u,v)$. [Hint: $D\vec r$ is a 3-row by 2-column matrix]
 \item  The vector field $\vec F(x,y) = (-y,xe^{3y})$.  Compute $\vec F_x$, $\vec F_y$, and $D\vec F(x,y)$.  [Hint: $D\vec F$ is a 2 by 2 matrix]
\item The parametric curve $\vec r(t)=(t,\cos t,\sin t)$. Compute $\vec r_t$ and $D\vec r(t)$.
\item The parametric surface $\vec f(u,v)=(u^2,v^2,u-v)$.  Compute $\vec f_u$, $\vec f_v$, and $D\vec f(u,v)$.
\item The space transformation $\vec T(r,\theta,z)=(r\cos\theta, r\sin\theta, z)$. Compute $\vec T_r$, $\vec T_\theta$, $\vec T_z$, and $D\vec T(r,\theta, z)$.
\end{enumerate}
\end{problem}
...
\end{document}
\section{Geometry and the derivative}
