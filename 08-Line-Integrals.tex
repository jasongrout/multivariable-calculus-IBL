...
We're not prepared to compute both work (circulation, flow) and flux.  The next 4 problems ask you to do so. The most common way to remember these, provided the vector field is $\vec F(x,y) = (M(x,y),N(x,y))$, is 
$$\text{Work}=\int_C M dx+N dy\quad\quad \text{Flux}=\int_C N dx-M dy.$$
