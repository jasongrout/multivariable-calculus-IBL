...
\note{This problem needs modifications}
\begin{problem}
...
%Change the third bullet to the following
% \item We defined $\vec B=\vec T\times \vec N$. Find $\vec N\times \vec T$, $\vec B\times \vec T$, $\vec T\times \vec B$, $\vec B\times \vec N$, and $\vec N\times \vec B$. 
\end{enumerate}
\end{problem}
...
\note{The next 2 problems here need to change slightly. First, I want the paragraph below to be part of a problem that they do. Here's an idea for the next 2.
...
\begin{enumerate}
 ...
 \item If $\vec B=(0,0,1)$, find $\vec N$ using an appropriate cross product. If instead $\vec B=(0,0,-1)$, what does $\vec N$ equal?
...
\begin{enumerate}
 ...
 \item Now find $\vec N(t)$ by using an appropriate cross product (the last problem is the key).
...
\note{One more problem here might be good.  The idea would be the following.  Tell them the curvature vector and a speed.  Then have them compute T'(t) (something they did in part 5 above).  Then ask them to forget k for a second and then using their new knowledge of T'(t) and speed, find the curvature vector.  Then have them keep curvatuve the same, and change the speed and T'. Again, use this knowledge to find curvature.  I somehow want them to realize that curvatuve is a physical property of a curve, and doesn't depend on speed at all.  It serves as  a reference vector, from which you can determine T'(t) for any time, provided you know the speed. I also want them to know how to get from one to the other rapidly (divide or multiply by speed).  They don't get this idea when I put it inside a paragraph (like I did below).  That's one of the main things I've learned this semester. They don't internalize (or even read) the paragraphs between problems.}
